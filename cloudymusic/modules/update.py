...
from cloudymusic.core import chat_cache, call, Filter, config
from cloudymusic.logger import LOGGER
from cloudymusic.modules.utils.play_helpers import del_msg
...
async def update(c: Client, message: types.Message) -> None:
    ...
    if is_docker():
        ...
        cloudymusic_path = shutil.which("cloudymusic")
        if not cloudymusic_path:
            await msg.edit_text("❌ Unable to find 'cloudymusic' in PATH.")
            return
        execvp("cloudymusic", ["cloudymusic"])
